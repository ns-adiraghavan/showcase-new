// ─── /research/tech2 — Technology industry filtered view (alt layout v2) ──
// Adapted from TechAlt-full.jsx (/research/tech1).
// Changes vs tech1:
//   1. InsightBoxes takes the full container width (no panel landing split)
//   2. No sample count numbers rendered in the filter results bar
//   3. Individual bottleneck/solution points have a hover highlight effect
...
// ─── Hoverable row — individual point highlight on hover ──────────
function BottleneckRow({ item, accent, isLast }) {
  const [hov, setHov] = useState(false);
  return (
    <div>
      {!isLast && (
...
function SolutionRow({ item, isLast }) {
  const [hov, setHov] = useState(false);
  return (
    <div>
      {!isLast && (
...
// ─── Two-box insight section — full width ─────────────────────────
// Change vs tech1: maxWidth removed from wrapper so boxes span the
// full container width set by the parent padding context.
function InsightBoxes({ accent }) {
  return (
    <div style={{ maxWidth:1160, margin:"0 auto", padding:"0 clamp(16px,4vw,44px)", marginBottom:36 }}>
      <div
        style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:0, borderRadius:4,
          overflow:"hidden", boxShadow:"0 2px 16px rgba(0,95,134,0.10)" }}
        ...
        <div style={{ background:NS.surface, padding:"28px 30px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column" }}>
            {BOTTLENECKS.map((item, i) => (
              <BottleneckRow key={i} item={item} accent={accent} isLast={i === BOTTLENECKS.length - 1} />
            ))}
          </div>
        </div>
        ...
        <div style={{ background:accent, padding:"28px 30px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column" }}>
            {SOLUTIONS.map((item, i) => (
              <SolutionRow key={i} item={item} isLast={i === SOLUTIONS.length - 1} />
            ))}
          </div>
        </div>
      </div>
...
// ─── Root ─────────────────────────────────────────────────────────
export default function TechAlt2() {
  ...
  return (
    <div style={{ background:NS.paper, minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:1160,margin:"0 auto",padding:"0 clamp(16px,4vw,44px)" }}>
        ...
        {/* ── No sample count rendered (tech2 change) ── */}
